...
def main():
    ...
    dataset_X, dataset_y = data_test[:, :2], copy.deepcopy(data_test[:, 2]).astype(int)
    plot_data(data_test, "true")

    outputs = model.calculate_forword(dataset_X)
    classfication = classify(outputs[-1])
    data_test[:, 2] = classfication
    plot_data(data_test, "after training")
