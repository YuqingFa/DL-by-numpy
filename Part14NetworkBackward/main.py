...
def main():
    ...
    outputs = model.calculate_forword(dataset_X)
    classification = classify(outputs[-1])
    data[:, 2] = classification
    plot_data(data, "before training")

    backup_network = model.network_backward(outputs, dataset_y)
    new_outputs = backup_network.calculate_forword(dataset_X)
    new_classfication = classify(new_outputs[-1])
    data[:, 2] = new_classfication
